...
Considerations when implementing a Hash Table:
	* How do we convert the key into an integer
	  value (the hash)?
	* What happens when we try to store a KVP in an
      already full slot (collision)?
	* What are appropriate sizes of the underlying vector?
	* How does performance change as our underlying
	  vector fills up?
